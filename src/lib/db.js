...
export const accountToRow = (a) => ({
  ...
  rendimento_categoria_id: a.rendimentoCategoriaId || null,
  rendimento_modo: a.rendimentoModo || 'conta',
...
export const rowToAccount = (r) => ({
  ...
  rendimentoCategoriaId: r.rendimento_categoria_id || null,
  rendimentoModo: r.rendimento_modo || 'conta',
...
export async function loadFromDb(defaultData) {
  try {
    ...
    // Guarda contra cache. `served_at` é o momento em que o servidor montou a resposta; uns
    // segundos de diferença (inclusive negativa) são só desvio de relógio. Um número grande
    // significa resposta reaproveitada do cache — e aí a tela mostra uma foto velha do banco,
    // que é indistinguível de "não salvou". Avisa SÓ nesse caso: em operação normal, silêncio.
    const idadeSeg = d.served_at ? Math.round((Date.now() - new Date(d.served_at)) / 1000) : null
    if (idadeSeg != null && idadeSeg > 10) {
      console.warn(`[db] /api/load veio de cache — montada há ${idadeSeg}s`)
    }
    // Resíduo do diagnóstico de persistência, já encerrado: limpa a chave de quem ainda a tem.
    try { localStorage.removeItem('finup_debug_ultimo_sync') } catch { /* indisponível */ }
